...
The specialization project would explore ID and biodiversity with the same dataset used in the original paper (the authors have made it [publicly available](https://impact.earthdata.nasa.gov/casei/campaign/SHIFT/#data)), using new methods of determining intrinsic dimensionality, such as the Generalized Ratios Intrinsic Dimension Estimator (GRIDE). I've used GRIDE a bit with hyperspectral images. It is fairly straightforward to apply and I have some (fairly slow) code already. Your task would be to assess whether the apparent relationship between biodiversity and ID persists even when a different technique is used to estimate ID, and to think about the limits of the connection, such as which ecosystems it applies to.
